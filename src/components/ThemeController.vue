...
<template>
  <div title="Change Theme" class="block dropdown dropdown-end">
    <div
      tabindex="0"
      role="button"
      class="group gap-1.5 px-1.5 btn btn-sm btn-ghost"
      aria-label="Change Theme"
    >
      <div
        class="gap-0.5 grid grid-cols-2 bg-base-100 p-1 border border-base-content/10 group-hover:border-base-content/20 rounded-md transition-colors shrink-0"
      >
      ...
      <svg
        width="12px"
        height="12px"
        class="hidden sm:inline-block opacity-60 fill-current mt-px size-2"
    ...
    <div
      tabindex="0"
      class="top-px z-50 bg-base-200 shadow-2xl mt-16 border border-white/5 rounded-box outline outline-1 outline-black/5 h-[30.5rem] max-h-[calc(100vh-8.6rem)] overflow-y-auto text-base-content dropdown-content"
    >
      <ul class="w-56 menu">
        <template v-for="(themes, groupName) in themeGroups" :key="groupName">
          ...
          <li v-for="theme in themes" :key="theme">
            ...
            >
              <div
                :data-theme="theme"
                class="gap-0.5 grid grid-cols-2 bg-[oklch(var(--b1))] shadow-sm p-1 rounded-md shrink-0"
              >
              ...
              <svg
                ...
                :class="[
                  'w-3 h-3 shrink-0',
                  currentTheme === theme ? 'visible' : 'invisible',
                ]"
              >
                <path
